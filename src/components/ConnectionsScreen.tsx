...
function isSameConnections(a: ConnectionWithState[], b: ConnectionWithState[]) {
  ...
  return a.every((conn, index) => {
    ...
    return (
      ...
      conn.contactMobileNumber === other.contactMobileNumber &&
    ...
    const connectionsWithState = await Promise.all(
      rawConnections.map(async (conn) => {
        ...
        return {
          ...
          totalTradedAmount,
          lastActivityAt,
          contactMobileNumber: otherBusiness?.mobileNumber ?? null,
        }
      })
    )
                  ...
                  {conn.contactMobileNumber && (
                    ...
                    >
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                          window.location.href = 'tel:' + conn.contactMobileNumber
                        }}
                        className="flex items-center justify-center"
                        style={{ width: '32px', height: '28px', backgroundColor: '#2D2D2D' }}
                        aria-label="Call"
                      >
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                          window.open('https://wa.me/' + conn.contactMobileNumber!.replace('+', ''), '_blank')
                        }}
                        className="flex items-center justify-center"
                        style={{ width: '32px', height: '28px', backgroundColor: '#25D366' }}
                        aria-label="WhatsApp"
                      >
